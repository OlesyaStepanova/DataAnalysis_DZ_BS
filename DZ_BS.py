...
def getFlats(page):
    ...
    for data in soup.find_all("div", attrs={"data-testid" : "offer-card"}):
        data2 = data.find("a", class_="_93444fe79c--link--VtWj6")
        link = data2['href']
        ...
        if geo:
            geo = geo
            metro = geo.find("a", class_="_93444fe79c--link--BwwJO")
            if metro:
                metro = metro.text
            else:
                metro = "Информация отсутствует"
            how_to_go = geo.find("div", class_="_93444fe79c--remoteness--q8IXp")
            if how_to_go:
        ...
        address = data.find("div", class_="_93444fe79c--labels--L8WyJ")
        if address:
...
def main():
    ...
    df.to_csv('flats1.csv', index=False, encoding='utf-8')
